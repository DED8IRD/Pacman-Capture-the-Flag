...
class SmartAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if time.time() - start > 0.9: print 'eval time for agent %d: %.4f' % (self.index, time.time() - start)
...
class DefensiveAgent(SmartAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if len(invaders) > 0:
      dist = min([self.getMazeDistance(myPos, invader.getPosition()) for invader in invaders])
      if dist == 0: dist = -100
      features['invaderDistance'] = min([self.getMazeDistance(myPos, invader.getPosition()) for invader in invaders]) + 1
